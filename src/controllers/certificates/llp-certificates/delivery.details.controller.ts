...
import {DeliveryDetailsPropertyName} from "./DeliveryDetailsPropertyName";
...
export const setBackLink = (certificateItem: CertificateItem, session: Session | undefined): string => {
    ...
    } else if (certificateItem.itemOptions?.memberDetails) {
        path = "members-options";
    } else if (certificateItem.itemOptions?.designatedMemberDetails) {
