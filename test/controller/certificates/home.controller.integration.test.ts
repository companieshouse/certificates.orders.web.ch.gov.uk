...
describe("certificate.home.controller.integration", () => {
    ...
    it("configurable banner does not override 'There is a problem' on start now when basket full", async () => {
        // Enable configurable banner
        process.env.CONFIGURABLE_BANNER_ENABLED = "true";
        process.env.CONFIGURABLE_BANNER_TITLE = "Configurable banner title";
        process.env.CONFIGURABLE_BANNER_TEXT = "This is configured banner text.";

        // Recreate app so config is reloaded with the env change
        testApp = getAppWithMockedCsrf(sandbox);
        ...
        chai.expect(resp.status).to.equal(200);
        // basket-full problem must take priority and show error
        chai.expect(resp.text).to.contain(`Basket (${BASKET_ITEM_LIMIT})`);
        chai.expect(resp.text).to.contain(`There is a problem`);
        chai.expect(resp.text).to.contain(`Your basket is full. To add more to your order, you&#39;ll need to remove some items first.`);
        // start now should be disabled
        verifyStartButtonEnabledStateIs(resp.text, false);
    });
