...
describe("llp-certificates.home.controller.integration (configurable banner)", () => {
    beforeEach(() => {
        ...
        describe(scenario.name, () => {
            ...
            it(`banner shown = ${scenario.expectBannerShown}`, async () => {
                ...
                const resp = await chai.request(testApp)
                    .get(replaceCompanyNumber(LLP_ROOT_CERTIFICATE, mockCompanyProfileConfiguration.companyNumber))
    ...
    it("configurable banner does not override 'There is a problem' on start now when basket full", async () => {
        ...
        try {
            getCompanyProfileStub = sandbox.stub(CompanyProfileService.prototype, "getCompanyProfile")
                .resolves(getMockCompanyProfile({ companyType: "llp", companyStatus: "dissolved" }));
            sandbox.stub(apiClient, "getBasket").resolves(getDummyBasket(true, BASKET_ITEM_LIMIT));

            const url = replaceCompanyNumber(LLP_ROOT_CERTIFICATE, mockCompanyProfileConfiguration.companyNumber) + START_BUTTON_PATH_SUFFIX;
            const resp = await chai.request(testApp)
                .get(url)
                .set("Cookie", [`__SID=${SIGNED_IN_COOKIE}`]);
