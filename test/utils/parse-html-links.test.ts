...
describe("parseHtmlLinks", () => {
  ...
  it("converts a single markdown link", () => {
    ...
    const expected = 'See <a class="govuk-link" href="https://www.gov.uk">GOV.UK</a> for details';
  ...
  it("converts multiple markdown links in the same string", () => {
    ...
    const expected = 'One <a class="govuk-link" href="https://a.test">A</a> and <a class="govuk-link" href="https://b.test">B</a>';
    expect(parseHtmlLinks(input)).to.equal(expected);
  });

  it("handles URLs containing parentheses", () => {
    const input = "See [example](https://example.test/path(foo)) for details";
    const expected = 'See <a class="govuk-link" href="https://example.test/path(foo)">example</a> for details';
    expect(parseHtmlLinks(input)).to.equal(expected);
  });
  ...
  it("handles links where URL contains newlines", () => {
    ...
    const expected = 'Broken <a class="govuk-link" href="https://example.test/line1\nline2">link</a> end';
    expect(parseHtmlLinks(input)).to.equal(expected);
  });
  ...
  it("does not crash and preserves other HTML-like content", () => {
    ...
    const expected = "<script>alert('x')</script> and <a class=\"govuk-link\" href=\"https://ok\">Link</a>";
    expect(parseHtmlLinks(input)).to.equal(expected);
  });
  ...
  it("handles empty link text and empty url (edge case)", () => {
    ...
    const expected = 'Empty <a class="govuk-link" href=""></a> here';
    expect(parseHtmlLinks(input)).to.equal(expected);
  });
